...
> **Otros documentos del proyecto:** `State_GAS.md`, `State_Abilities.md`, `State_Combat.md`, `State_AI.md`, `State_Input.md`, `State_UI.md`, `State_Progression.md`, `State_Pending.md`, `State_Validation.md` (validación de Data Assets), `State_Hardening.md` (campaña de robustez), `State_Architecture_Future.md` (**diseño prospectivo** del árbol/corazones/loadout — leerlo antes de crear habilidades o nodos nuevos), `State_DeveloperTools.md` (**el plugin PDS** — herramientas de inspección/validación dentro de Unreal). Cuando un sistema de `State_Pending.md` se implementa, se documenta en el archivo temático que le corresponde (p. ej. el parry y el melee del jugador → `State_Combat.md`; el daño elemental → `State_Abilities.md`/`State_GAS.md`; los niveles y el árbol → `State_Progression.md`; la IA de bosses → `State_AI.md`) y se elimina de Pending.

> **Panthelia Developer Suite (PDS) — disponible para todos los chats.** El proyecto incluye un plugin **Editor-only** en `Plugins/PantheliaDeveloperSuite`, validado para UE 5.8. Su Automation API `0.5.0-alpha2` expone **siete tools MCP** para Codex **dentro de Unreal**: `GetStatus`, `ListSnapshots`, `ExportProjectSnapshot`, `ValidateProfile`, `SetLatestSnapshotAsBaseline`, `CompareLatestSnapshotWithBaseline` y `CompareLatestTwoSnapshots`. También ofrece **dashboard** (`Tools → Panthelia Developer Suite`), **Project Doctor** (perfiles `PantheliaCore` / `GameContent` / `ExternalContent` / `EntireProject`), **Montage Inspector** y **Snapshot Diff Browser**. **Antes de pedir inspección manual extensa al usuario, comprobar si PDS puede obtener el dato de forma estructurada.** PDS **no corrige ni guarda `.uasset`**; sus snapshots y reportes se escriben bajo `Saved/PantheliaDeveloperSuite`. Usar **snapshots + diffs** para evidenciar qué cambió en una fase, y **validación por perfil** antes de cerrar trabajo. Distinguir **Codex de escritorio** (C++, compilar con Unreal cerrado, Git) de **Codex/MCP dentro de Unreal** (las siete tools). El único dominio semántico actual es `spells@1.0.0` y cubre solo hechizos de proyectil concretos. Detalle en `State_DeveloperTools.md`.
