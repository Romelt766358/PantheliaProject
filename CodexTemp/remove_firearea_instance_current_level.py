...
FIRE_AREA_CLASS = "/Game/Blueprints/GameplayEffects/BP_FireArea.BP_FireArea_C"
...
targets = [
    ...
    if actor.get_class().get_path_name() == FIRE_AREA_CLASS
]

if len(targets) != 1:
    raise RuntimeError(
        "Expected exactly one BP_FireArea instance, found %d" % len(targets)
    )
...
unreal.log("BLOCK5B_REMOVED_FIREAREA=" + target_path)
unreal.log("BLOCK5B_SAVED_LEVEL=" + EXPECTED_LEVEL)
